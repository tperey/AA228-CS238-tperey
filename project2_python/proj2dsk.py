...
if __name__ == "__main__": 
    ...
    # Let's try some domain specific knowledge!
    # Initially, small states (far from flag), should have lower Q. Should essentially match
    # state = 1+pos+500*vel
    velocities = 100 # Number of possible velocities (groups of positions)
    positions = len(S)/velocities # Number of initial state values (velocities)
    actions = len(A)
    values = np.arange(positions)  # Create vector 0 to positions (diff value for each position). Further right positions better

    # # Position 465 (index 464) is best, so scale those after it back down
    # subtractor = 2
    # for i in range(465,len(values)):
    #     values[i] -= subtractor
    #     subtractor += 2
    single_action = np.tile(values, velocities).reshape(-1,1) # Stack these for each velocity
    Q = np.repeat(single_action, actions, axis = 1) # Spread over all actions
    Q = 200*Q # Scale up, so initial has more of an effect
